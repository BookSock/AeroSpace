...
%{window-native-spaces}:: String. Native macOS Space IDs that contain the window. Empty unless `experimental-native-spaces` is enabled. `unavailable` if native Space membership could not be read.
...
%{native-space}:: String. Native macOS Space namespace of the belonging workspace
